...
// 标签映射函数
export const mapTags = (tags: string[], gender: 'male' | 'female'): string[] => {
...
export const charactersData: Record<string, Character> = {
  "1": {
    ...
    description: "The talented swimmer who dominates the competition. But you're the only one who knows about your secret romance. Will he ever make it public, or will your love remain hidden forever?",
    chatCount: "1.4M",
    likeCount: "13.0K",
    followers: "1.1M",
    imageSrc: "/alexander_avatar.png",
    avatarSrc: "/alexander_avatar.png",
  ...
  "2": {
    ...
    description: "The successful CEO who knows exactly what he wants. Behind his confident exterior lies a man with desires only you can fulfill. Will you be the one to break through his walls?",
    chatCount: "3.9K",
    likeCount: "9.5K",
    followers: "2.3M",
    imageSrc: "/alexander_avatar.png",
  ...
  "3": {
    ...
    description: "The charming plumber with a playful smile and skilled hands. What started as a routine house call has turned into something much more exciting. He's ready to fix more than just your pipes.",
    chatCount: "1.7K",
    likeCount: "7.6K",
    followers: "980K",
    imageSrc: "/alexander_avatar.png",
  ...
  "5": {
    ...
    description: "A shy and repressed girl, hiding her deep attraction to you while trying to fit in. Behind her innocent facade lies passionate desires waiting to be discovered.",
    chatCount: "24.1K",
    likeCount: "193",
    followers: "2.4M",
    imageSrc: "/alexander_avatar.png",
  ...
  "6": {
    ...
    description: "A reserved mother with a hidden perverted side, she will do anything to satisfy your desires. Your new neighbor with secrets she's dying to share.",
    chatCount: "7.0K",
    likeCount: "204",
    followers: "1.5M",
    imageSrc: "/placeholder.svg?height=200&width=150&text=Anshu-1",
    avatarSrc: "/avatar/female_03_avatar.png",
    images: ["/placeholder.svg?height=200&width=150&text=Anshu-1", "/alexander_avatar.png"],
    gender: "female",
    creator: { id: "mei-chan", name: "Mei chan", likeCount: "204" },
  },
  "7": {
...
// 核心函数：获取角色列表（支持筛选、分页等）
export const getCharacters = (options: CharacterFilterOptions = {}): CharacterListResponse => {
  const {
    gender = 'all',
    activeTag = 'For You',
    page = 1,
  ...
  if (activeTag !== 'For You' && activeTag !== 'Popular') {
    charactersList = charactersList.filter(char => 
  ...
  charactersList.sort((a, b) => {
    switch (sortBy) {
      case 'popular':
        return parseInt(b.chatCount.replace(/[^\d]/g, '')) - parseInt(a.chatCount.replace(/[^\d]/g, ''));
      case 'newest':
...
// 获取趋势角色
export const getTrendingCharacters = (gender: 'male' | 'female' = 'male', limit: number = 5): Character[] => {
